...
> **모노레포 구조 (2026-06-17~)**: 앱은 `apps/games/` 에 위치 (Turborepo, planner/Q 와 동형 토폴로지). 본 문서의 `apps/games/...`·`apps/games/scripts/...` 등 앱 경로는 **모노레포 루트 기준**. dev/build/test 는 루트에서 turbo 로 실행 (`bun dev` = `turbo dev`, `bun run build`·`bun run test`). backend·packages 는 thin monorepo 정책상 미생성 — BE 는 별 repo `pullim-api`. 근거: `proc/plan/2026-06-17_monorepo-restructure.md`.

## 1. 도메인 범위

| 영역 | 경로 |
|---|---|
| **게임 라우트** | `apps/games/app/games/`, `apps/games/app/manage/` (관리) |
| **게임 카탈로그** | `apps/games/games/{21개 게임}/` |
| **메커니즘 컴포넌트** | `apps/games/components/game-mechanics/{Blank,QuickQuiz,Typing,WordMatch}Component.tsx`, `useAttemptCounter.ts` |
| **게임 셸·허브** | `apps/games/components/{game-shell,game-hub,dashboard,GameCard,RecommendationCard}/` |
| **공유 lib** | `apps/games/lib/{core,games,server,utils.ts}/` |
...
| 영역 | 경로 | 주의 |
|---|---|---|
| 게임 셸 | `apps/games/components/shell/`, `apps/games/components/game-shell/` | 21개 게임 공통 셸 — 수정 시 전 게임 영향 |
| UI 프리미티브 | `apps/games/components/ui/` | shadcn/ui + Radix |
| 디자인 시스템 | `proc/spec/08-디자인-시스템.md` (read only) | |
| 게임 lib | `apps/games/lib/core/`, `apps/games/lib/games/` | FSRS·checkAnswer 등 핵심 로직 |
...
- 21개 게임 중 단일 게임 작업: `apps/games/games/<game-name>/` 신규·수정
- 4 메커니즘 컴포넌트 보강 (`apps/games/components/game-mechanics/`)
- `apps/games/games/<game-name>/`에서 메커니즘 import해서 콘텐츠·스키마·distractor 추가
...
- `apps/games/lib/core/`, `apps/games/lib/games/` 공통 로직 수정 (checkAnswer, FSRS 등)
...
- **브랜치 승격**: 최신 `dev`에서 작업 브랜치 생성 → PR로 `dev` 머지·dev 배포 확인 → `dev`에서 `main`으로 승격 PR → production 배포. `dev`·`main` 직접 push 금지. 상세 순서는 `README.md`의 "개발·승격 흐름"을 따른다.
...
| 상황 | 명령 |
|---|---|
| 개발 (dev) | `bun dev` → http://localhost:**3004** (포트 SoT: `proc/spec/09 §9.1`. 옛 3033 → 3004 cutover 2026-06-29) |
...
PR 자동 AI 리뷰 봇(`.github/workflows/codex-review.yml`, Codex Review)은 **2026-09-15 폐지**됐다(계획·근거: pullim-api `docs/common/2026-09-15-codex-리뷰-워크플로-제거/`). 현재 PR 게이트는 `ci.yml` + 사람 리뷰다. 검증 결과의 신뢰성을 지키는 원칙은 봇 유무와 무관하게 유지한다 — **사용자 합의 2026-05-20**.

### 원칙 — "검사관을 매수하지 마라"

리뷰어(사람·AI 에이전트)의 룰북을 claude 가 임의로 수정해서 지적을 회피하는 행위는 **작업 결과의 신뢰성을 오염**시킨다.

### 해야 하는 것

- **리뷰 지적은 원칙적으로 코드 fix 로 응답.** 룰북(AGENTS.md·CLAUDE.md·proc/spec/01~10) 을 **회피 목적**으로 수정하는 행위 금지. 지적이 실제 명세 결함을 짚은 경우는 회피가 아니라 정당한 명세 진화 — `proc/spec/01-AI-명령지침.md §2 명세 우선 원칙` 경로(별 plan 에 근거 기록 → 사용자 합의 → spec 먼저 수정 → 코드 fix)로 정정한다. 거버넌스 정착 근거: `proc/archive/plan/2026-05-20_plan-g-pullim-workflow-port.md`.
- **사전 sweep 의무 (PR 생성 전):** 권위 문서(`proc/spec/01~10`) 룰 위반 점검(특히 `01-AI-명령지침.md` §3 코드 정책·§7 문서 라우팅, `09-기술-환경.md` 보안·런타임 검증) + 보안 boilerplate 점검(secret 노출·외부 입력 런타임 검증·`safety_strategy=unsafe` 정당화).
- **정당한 trade-off 는 별 plan 합의 후 기록.** `KNOWN-TRADE-OFF: <근거 plan 경로>` 패턴으로 코드/주석에 명시.

### 하면 안 되는 것

- 룰북(AGENTS.md / CLAUDE.md / proc/spec) 을 리뷰 지적 **회피 목적으로** 수정 — 명세 자체 결함을 spec/01 §2 경로로 고치는 것과는 구분
- 사전 sweep 없이 PR 띄우고 지적 받은 뒤 룰북 쪽 수정으로 우회
- "회피"와 "정당한 명세 수정" 의 판단은 **별 plan + 사용자 합의 유무** 로 가른다 — claude 단독 판단 금지

### 거버넌스 위반 시

- claude 가 룰북 회피 수정을 시도하면 사용자가 **즉시 정정 지시** → 해당 변경은 revert, 코드 fix 로 재응답
