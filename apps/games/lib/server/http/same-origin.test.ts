...
function req(headers: Record<string, string>): Request {
  return new Request("http://localhost:3004/api/auth/login", { method: "POST", headers });
}
...
describe("isSameOriginRequest (dev — request.url fallback 신뢰)", () => {
  ...
  it("Origin 이 요청 origin 과 일치하면 허용", () => {
    expect(isSameOriginRequest(req({ origin: "http://localhost:3004" }))).toBe(true);
  });
  it("Origin 불일치면 거부", () => {
    expect(isSameOriginRequest(req({ origin: "https://evil.example.com" }))).toBe(false);
  });
  it("Origin 없고 Referer 가 일치 origin 이면 허용", () => {
    expect(isSameOriginRequest(req({ referer: "http://localhost:3004/login" }))).toBe(true);
  });
  it("Referer 가 외부면 거부", () => {
