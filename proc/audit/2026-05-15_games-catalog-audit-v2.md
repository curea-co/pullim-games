...
- factorization: BUG-2 fix (drag 자유 방향 + block 위치 hit-test) 머지 완료. 단 메커닉 차원 변별력은 여전히 0 (모든 term 드래그 → success). → **별 plan** `proc/plan/2026-05-14_factorization-discrimination.md` (drag-to-chip 메커닉 재설계) 로 분리 처리 중.
...
- **factorization 변별력 강화** — `proc/plan/2026-05-14_factorization-discrimination.md` Phase 1 (schema/distractor 생성) 진입 예정.
- **CorrectBurst polish (V0.1)** — 폭죽/사운드는 보류, 시각 톤 dogfooding 후 재검토.
- **production 배포** — Vercel webhook 우회 (`vercel --prod` 수동). PR #45/#46/#47 머지분 사용자 수동 배포 대기.
...
| 변별력 정책 준수 (코드 기준) | 21/21 (audit miss) | 20/21 (factorization 별 plan) | 20/21 (동일) |
| Health score 추정 | 92.5 | 99 | **99+** (LOW 2건 해소) |
