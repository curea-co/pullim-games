...
> 대상: **SRS(간격반복 카드상태) · 스트릭 · 활동로그(activity_log) · 커스텀 콘텐츠** 4종.
> (활동로그는 대시보드 활동량 — D8상 14일 retention, 나머지 3종은 무기한. 전 문서 4종 기준.)
...
계정 사용자의 **SRS 카드상태 · 스트릭 · 활동로그(activity_log) · 커스텀 콘텐츠**(4종)를 games 전용 Postgres 에 영속하여, **기기를 바꾸거나 브라우저를 클리어해도 로그인하면 학습 진도가 복구**되게 한다. auth phase 에서 깔아둔 `fingerprint_links`(first-writer-wins 토대)를 활용해 **로그인 시 1회 익명 데이터 흡수**를 수행한다.
...
| D5 | **익명→계정 흡수** | (a) 자동 / (b) 안 함 / (c) **in-session 전환만 흡수** | ✅ **(c) 확정** — 공유 기기(학교 PC) 명의오염 차단. ⚠️ 핵심(R8): **로그인 전 익명 로컬 데이터의 소유권은 원리상 증명 불가** — localStorage 토큰은 자기 자신과 비교라 무의미(학교 PC 잔존 데이터 통과). → **흡수 대상 = "같은 JS 세션 내 게스트 플레이 → 가입/로그인 전환"만**(in-memory 세션 플래그, localStorage 아님). 즉 *방금 이 세션에서 게스트로 놀다가 가입한 흐름*에서만 흡수 제안. **이전 브라우저 세션의 잔존 localStorage(학교 PC 시나리오)는 자동 흡수 안 함**(in-memory 플래그 부재). 추가로 ① `fingerprint` 타 계정 소유면 차단. 그 후 확인 프롬프트 → 동의 시에만, **머지 경유**(§5 P5) |
...
-- 대시보드 조회: SELECT game_id, date, SUM(count) ... WHERE user_id=$1 GROUP BY game_id, date
--   (game_id 차원 보존 — 게임별 heatmap·loadActivity(gameId) 호환, R7).
...
| P3 | 동기화 API | `src/app/api/sync/route.ts` — **GET=증분 pull**(전체 아님), **POST=push 배치 델타(커스텀은 스냅샷 전량 포함)**. 단일 메서드 계약(PUT 미사용). zod 검증 + **401 인증 게이트**. 모든 쿼리 `WHERE user_id=me.id`(cross-user 0). **fingerprint 미사용(R5)**: sync write 키는 `device_id`(랜덤 UUID)뿐 — fingerprint 가 write 에 안 들어가므로 `fingerprint_links` first-writer-wins 소유권이 sync 로 훼손되지 않음(소유권은 흡수 단계서만 검사). **⏱ 증분 cursor = 단조 시퀀스(#117 R8)**: srs/streak/custom 의 `updated_at` 은 **서버 시퀀스(`nextval`)로 stamp** — 벽시계 ms 가 아니라 행마다 고유·단조. 같은 ms 에 두 write 가 같은 cursor 를 받아 다음 pull 이 영구 누락하던 충돌 방지. **클라는 GET 응답의 리소스별 cursor 로만 증분 cursor 를 전진**(POST 는 cursor 미반환 — no-op 된 stale 이 cursor 를 미래로 올려 더 최신 행을 건너뛰는 footgun 차단). LWW/recency 는 별도(`last_review_at`/`exportedAt`), retention 은 `date` 기준 — `updated_at` 은 cursor 전용. **증분 pull 계약(R1/R2)**: 클라가 리소스별 `since`(마지막 GET 이 준 cursor)를 보내고 서버는 **단일 200 응답에 리소스별 변경분 + unchanged 마커**로 반환 — `{ srs:{changed:[...]}, custom:{unchanged:true}|{snapshot,...}, streak:..., activity:... }`. (HTTP 304 미사용 — 304는 응답 전체 상태라 "커스텀 unchanged + SRS changed" 공존 불가, R2.) 커스텀은 `updated_at` 미변경 시 `{unchanged:true}` 로 4MB 재전송 0. 리소스별 분리라 게임 오갈 때 대용량 반복 다운로드 없음. 🔒 **CSRF 가드(POST)**: 기존 `src/lib/server/http/csrf.ts` **stateless double-submit 쿠키** 헬퍼 재사용(billing HttpOnly nonce 아님 — 백그라운드 반복 flush엔 재사용 가능한 double-submit이 적합, R3). 흐름: `GET /api/sync/csrf`(또는 기존 `/api/auth/csrf`)가 토큰을 **non-HttpOnly 쿠키(Path=/, SameSite=Strict)** 로 set → 클라가 쿠키 읽어 POST `x-csrf-token` 헤더로 echo → 서버가 쿠키==헤더(constant-time) 검사. 1차 same-origin(Origin/Referer) + 2차 토큰 다층. 미적용 시 외부 사이트가 학습 상태 주입(R1). |
...
| P5 | 익명→계정 흡수(in-session 전환 한정) | **① in-session 게이트(R8)**: 흡수는 *같은 JS 세션에서 게스트 플레이 후 가입/로그인한 흐름*에서만(in-memory 플래그 `justConvertedFromGuest`, localStorage 아님). 이전 세션 잔존 데이터(학교 PC)는 플래그 부재로 자동 흡수 제외 — 로그인 전 익명 데이터 소유권은 증명 불가이므로 "방금 본인이 만든 세션"으로 범위를 좁힘. **② fingerprint 타 계정 소유면 차단**(R4). **③ 확인 프롬프트**(D5=c) → 동의 시에만. **④ blind upsert 아니라 머지 경유**. 멱등(`ON CONFLICT`+머지) |
...
- **흡수 in-session 게이트(R8)**: 이전 세션 잔존 localStorage(학교 PC, in-memory 전환 플래그 부재)는 자동 흡수 안 됨. 같은 세션 게스트→가입 흐름만 흡수(회귀 가드).
- **시계 권위(R7)**: fast-clock 기기가 미래 `updated_at` 올려도 LWW/`since` 왜곡 없음(서버 stamp 기준). canonical updated_at 반환으로 cursor 갱신.
- **activity game 차원(R7)**: 집계가 game_id별 보존 — 게임별 활동량 UI 반영(단일 total 뭉갬 회귀 가드).
...
| **공유 기기(학교 PC) 명의오염** — 익명 데이터가 남 계정에 흡수 | ✅ D5=(c) 확인 후 + **fingerprint 소유권 게이트(R4)**: 타 계정 소유 fingerprint 면 흡수 차단. 확인 프롬프트만으로는 부족 — first-writer-wins 를 흡수까지 관철. 자동 흡수 금지. 실DB 명의격리 보존 |
...
- **구현 진행**: P0(spec+plan)는 #116 머지. **P1~P3(마이그레이션·서버 머지 모듈·`/api/sync`·cleanup cron)은 #117 에 적재**(BE only). 남은 P4~P7(클라 엔진·흡수·미성년·E2E)은 기존 공유소스 수정 포함 → FREEZE 해제 + 다른 세션 충돌 점검 후 별 PR. **사용자 노출(런타임 동작 변화)은 P4 클라 연결 후** — 현재 서버 엔드포인트는 존재하나 호출하는 클라가 없음.
...
| 기존 자산 | 본 plan 재사용 |
|---|---|
| `fingerprint_links`(auth) | 흡수 **소유권 게이트**(R4): 흡수 전 fingerprint 가 미귀속/현재 user 소유인지 검사해 first-writer-wins 를 흡수까지 관철(타 계정 소유면 차단) + 확인 프롬프트(D5) |
