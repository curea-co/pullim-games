...
4. **확인 필요·P0 설계 TODO**: ① games authz scope, ② `/games/me` introspection vs `/auth/me` 재사용, ③ 게스트 흐름 vs dev KCB 강제, ④ **CORS allowlist/헤더 계약**(직접 호출 전제), ⑤ **동기화 동시성**(client_updated_at·커서 tie-breaker), ⑥ **계정 product+환경 격리**(audience·dev/prod 스코프), ⑦ **기존 회원 계정 마이그레이션**(중앙 identity 이관·이메일 충돌·재인증), ⑧ **쿠키 Domain 스코프**(`Domain=.pullim.ai` 명시 — 부모도메인 동일만으론 자동 공유 X), ⑨ **CSRF cross-subdomain**(double-submit 이 games↔api 직접호출에서 성립하는 토큰 모델) — 모두 착수 시 pullim-api 공동 확정(**핸드오프 [P0 설계 TODO]·§A** 참조). 본 plan 은 방향만, 세부는 단독 명세 X.
