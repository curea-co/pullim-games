...
- [x] `lib/core/player/index.ts` `GRADES`: `초1~고3` → **`중1·중2·중3`**. `Grade` 타입 자동 축소. `StartForm` 드롭다운 자동 중등화.
  - **마이그레이션(Codex #125 R2·R7·R12·R13·R14)**: 두 책임 분리 —
    ...
    - `createPlayer()`(신원 확립·게스트 전용): 쓰기 전에 **`resetGuestSession()`** 로 이전(무효화된/다른 사용자) 게스트 진행도를 비워 **클린 슬레이트** → 새 프로필이 이전 사용자 데이터를 이어받는 교차 사용자 노출 차단(R13). 회원은 createPlayer 미호출(로그인=clearPlayer, 진행도 보존 — 익명→회원 승계 영향 없음).
