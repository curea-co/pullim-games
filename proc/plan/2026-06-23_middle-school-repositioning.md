...
- [x] `lib/core/player/index.ts` `GRADES`: `초1~고3` → **`중1·중2·중3`**. `Grade` 타입 자동 축소. `StartForm` 드롭다운 자동 중등화.
  - **마이그레이션(Codex #125 R2·R7·R12)**: 구 grade(초·고)·손상 프로필·stale 쿠키(무프로필) 감지 시 `getPlayer()` 가 **`clearPlayer()`** 호출 — **프로필+`pullim_games_guest` 쿠키만** 정리해 stale 쿠키 split-brain(middleware 가 쿠키만으로 보호 라우트 통과) 차단. ⚠️ 학습 데이터(`pullim-games:*` SRS·스트릭·활동·커스텀)는 **건드리지 않는다** — 클라 sync 미연결(spec/05)이라 localStorage 가 유일 런타임 저장소 → 회원·게스트 모두 비가역 손실 위험(R12). 전체 wipe 는 명시적 "다른 사용자로 시작"(`resetGuestSession`, 사용자 확인)에만. `player.test.ts` 로 (정리=프로필·쿠키 / 학습데이터 보존) 잠금. (R11 에서 resetGuestSession 으로 격상했다가 회원 데이터 손실 위험으로 R12 에서 되돌림.)
...
- [x] **보관 게임: "집계는 전체, 발견 노출만 visible"(Codex #125 R1·R2·R4·R5 수렴)**:
  ...
  - **R8·R9·R10 (홈 대시보드 최종)**: `perGame`(활동 카드 목록) 규칙 = **보관 미플레이 게임만 제외, 이미 플레이한 보관 게임은 유지**(stage!=="high" || cardsTouched>0). → 보관 게임 학습자의 활동·재진입 링크 보존(R10, 라우트 유지와 정합) + 미플레이 고등 게임 discovery 차단(R2). 이러면 `gamesPlayed`(전체) = 카드 목록과 정합 → 헤더·EmptyDashboard 게이트·목록 모두 전체 `gamesPlayed` 기준(R8 의 "N개 만났는데 목록 빈"을 숨김 아닌 재진입 유지로 해소). R8 에서 도입한 `visibleGamesPlayed` 는 불요해져 제거.
  - `/about` 도 `kind==="official"` 추가(custom-* 게임이 소개 게임 수·메커닉 쇼케이스 부풀리지 않게).
  - 인증 grade 계약: signup·login·me 라우트 테스트 모두 응답 `user.grade` 잠금(R6·R10).
