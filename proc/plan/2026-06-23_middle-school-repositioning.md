...
- [x] `lib/core/player/index.ts` `GRADES`: `초1~고3` → **`중1·중2·중3`**. `Grade` 타입 자동 축소. `StartForm` 드롭다운 자동 중등화.
  - **마이그레이션(Codex #125 R2·R7·R12·R13·R14)**: 두 책임 분리 —
    ...
    - `createPlayer()`(신원 확립·게스트 전용): **프로필+쿠키 영속이 확인된 뒤** `clearLearningProgress()`로 이전 게스트의 **신원 종속 진행도만**(SRS·스트릭·활동·커스텀) 비워 **클린 슬레이트** → 교차 사용자 노출 차단(R13). 보존: 프로필 키(STORAGE_KEY) + **기기 단위 비용 가드(`llm-quota:*`/`llm-cache:*`)** — 안 그러면 새 게스트 재생성으로 일일 LLM 한도·캐시 우회(R17). 생성 실패(저장 거부·쿠키 차단)면 wipe 미도달 → 기존 진행도 보존(R15). 회원은 createPlayer 미호출(로그인=clearPlayer, 진행도 보존).
