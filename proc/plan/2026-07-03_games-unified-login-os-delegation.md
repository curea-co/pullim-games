...
- **P-A. 클라 auth 메타데이터 계약 (PR-2 blocker)** — 조사(2026-07-05) 후 **역할 분리**:
  ...
  - [ ] **학년(grade) = games-side**: grade 는 콘텐츠 preference(identity 아님)이고 중앙 signup 미수집·auth 스키마 부재(planner/q 각자 보유)라, **games 가 회원 grade 를 자체 수집·games projection 보관**(PR-2, pullim-api 요청 아님 — 중앙 결합 회피). 회원 로그인 후 grade 미보유 시 수집 UX — ⚠️ **게스트 `StartForm` 재사용 불가**(게스트 온보딩 전용: 신원 있으면 `/home` 리다이렉트 + `createPlayer` 게스트 생성) → 별도 회원용 grade 수집 컴포넌트/모드 신설이 PR-2 설계 선결(아래 PR-2 항목).
...
- [ ] **회원 grade games-side 수집(P-A ⑵)**: 회원 로그인 후 grade 미보유 시 games 가 자체 수집·games projection 보관. 학년별 콘텐츠 타게팅 연결. ⚠️ **StartForm 그대로 재사용 불가**(Codex #143): `StartForm` 은 게스트 온보딩 전용 — `useIdentity()` 로 신원 있으면 즉시 `/home` 리다이렉트하고 submit 시 `createPlayer()` 로 게스트 localStorage 신원을 만든다. 회원 흐름에 쓰면 폼이 안 열리거나 회원 위에 게스트 프로필 생성 → **별도 회원용 grade 수집 컴포넌트/모드 신설(또는 StartForm 책임 재설계)이 PR-2 설계 선결**. ⚠️ grade 서버 저장 = 회원 서버 데이터 존재 → **P-C 파기 전파 계약이 활성화 시점부터 필수**(§2-D P-C 정정).
