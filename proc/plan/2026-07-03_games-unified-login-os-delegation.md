...
- [ ] **회원 grade games-side 수집(P-A ⑵)**: 회원 로그인 후 grade 미보유 시 games 가 자체 수집(게스트 StartForm 재사용)·games projection 보관. 학년별 콘텐츠 타게팅 연결. ⚠️ grade 서버 저장 = 회원 서버 데이터 존재 → **P-C 파기 전파 계약이 활성화 시점부터 필수**(§2-D P-C 정정).
