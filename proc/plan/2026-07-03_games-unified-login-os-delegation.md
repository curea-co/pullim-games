...
> ⚠️ **PR-1 은 게이트·리다이렉트 인프라까지.** 회원 신원·데이터 페치 재배선(use-identity introspection·domain-fetch·projection)은 PR-2. pullim 모드 **활성화(env on)는 §2-D P-A/B/C 선행 필수** — 본 PR 머지 후에도 env 미설정이라 legacy 기본(런타임 무변경).
