...
**상태**: **구현 PR (진행) — G1 승인 완료(2026-05-29: D1 spec개정 승인 / D2 신규 games Supabase / D3 이메일+비밀번호). spec/05 §5.2·§5.6 개정 + auth 코드 구현이 본 PR(#110)에 포함됨. 코덱스 리뷰-수정 루프 진행 중.**
**브랜치**: `feat/auth-login-signup` (worktree `/private/tmp/pullim-games-auth`, base `main` 719d2b0)
**잔여 운영 항목**: prod Supabase DATABASE_URL 은 사용자가 Vercel env 에 등록(머지/배포 시 자동 마이그레이션). 이는 코드 머지 차단 사유가 아님.
...
> ✅ **갱신(2026-05-29): G1 합의 완료 → spec/05 §5.2·§5.6 개정 + 코드 구현을 본 PR(#110)에 반영했다.** (당초 "spec 미수정 제안서" 단계를 거쳐, 거버넌스 순서 — 별 plan(본 문서) → G1 합의 → spec 개정 → 코드 — 를 모두 통과한 상태.)
...
## 8. 차단 사유 — 해소됨 (2026-05-29)

1. ~~D1 미합의~~ → **해소**: G1 이 spec 개정 승인. spec/05 §5.2·§5.6 개정 + 코드 구현 완료.
2. ~~D2 미확보~~ → **부분 해소**: 신규 games Supabase 프로젝트 결정 + 자격증명 확보. prod DATABASE_URL 의 Vercel env 등록은 배포 운영 단계 항목(코드 머지 차단 아님 — 배포 시 자동 마이그레이션).

→ 코드 구현 완료. 현재는 코덱스 리뷰-수정 루프 진행 중(머지는 코덱스 게이트 + 사용자 결정).
