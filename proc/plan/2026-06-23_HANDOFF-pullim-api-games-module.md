...
- **쿠키 Domain 스코프 — 공유 vs 격리 긴장(미해결 계약)**: "부모도메인 동일 → 쿠키 자동 공유"는 **틀림**(자동 아님). 그렇다고 단순히 `Domain=.pullim.ai` 로 넓히면 **games↔api 뿐 아니라 `planner.pullim.ai`·`q.pullim.ai` 등 sibling 전부가 같은 세션 쿠키를 읽어** spec/05 §5.2 독립 계정·세션 격리가 **붕괴**한다(= 본문이 짚은 "세션 격리 붕괴"). 따라서 `Domain=.pullim.ai` 를 답으로 단정하지 않는다 — **계약으로 풀어야 할 긴장**: ⒜ host-scoped(좁은) 쿠키 + games 가 토큰을 명시 전송, ⒝ Domain 은 넓히되 **audience=`games` 스코프**로 sibling 이 읽어도 재사용 불가, ⒞ product 별 쿠키 네임스페이스 — 중 무엇으로 "games↔api 직접호출 인증은 되되 sibling 누수는 없게" 할지 pullim-api 와 확정. Domain·`Secure`·`SameSite`(cross-subdomain) 까지 묶어 계약(아래 product/환경 격리 항목과 연동). **⚠️ 미들웨어 게이트와의 충돌(필수 해소)**: games 회원 coarse gate(`apps/games/middleware.ts`)는 **`games.pullim.ai` 에서 세션 존재 쿠키를 읽을 수 있어야** 보호 라우트를 통과시킨다. 후보 ⒜(host-scoped to `api.pullim.ai`) 를 고르면 그 쿠키는 `games.pullim.ai` 에서 **안 보여** 로그인 회원이 보호 라우트에서 `/` 로 튕긴다 → 게이트 붕괴. 따라서 쿠키 스킴은 **`games.pullim.ai`-readable 세션 존재 신호**(게스트 `pullim_games_guest` 와 동형의 non-HttpOnly host-only presence 힌트 쿠키, PII 없음)를 **반드시 동반**해야 한다. **⚠️ 단 이 presence 쿠키는 `api.pullim.ai` 가 심을 수 없다** — 브라우저 쿠키 규칙상 응답 origin 은 **sibling host(`games.pullim.ai`) 전용 쿠키를 set 못 한다**(자기 host 또는 `Domain=.pullim.ai` 부모만 가능). 그러므로 presence 힌트는 **games 가 직접 set**(현행 게스트 쿠키와 동일 — games host-only, `Path=/`, `SameSite=Lax`, Domain 미지정): FE 가 `api.pullim.ai` 로그인 성공 후, **games 측 thin 경로(또는 post-login FE 훅)가 자기 도메인에 presence 플래그를 set** 한다. api 는 세션을 발급할 뿐 games 쿠키를 만들지 않는다. 쿠키 스코프·presence set 주체·미들웨어 게이트를 **함께** 확정(따로 결정하면 핸드오프 입력이 틀어진다).
...
- games 측 **얇은 proxy/middleware** 가 보호 라우트 진입 게이팅. **회원·게스트 두 진입을 분리**해 판별한다(spec/05 §5.2 게스트 우선 — 게스트도 보호 라우트 통과해야 함):
  - **회원 게이트(2단 방어 — coarse + 정밀)**: ⒜ **coarse(서버 미들웨어)**: `games.pullim.ai`-readable 세션 존재 신호로 raw 라우트 1차 통과/차단. ⒝ **정밀(클라)**: 만료/위조 판정은 클라 `RequireIdentity` 가 **`/auth/me` introspection** 으로 확정(현행 games 는 `lib/auth/client.ts` `getMe`/`getAuthState` → `/api/auth/me`). **⚠️ 전제 1**: coarse 의 "쿠키 1차 필터"는 `games.pullim.ai` 에서 읽히는 신호가 있어야 성립 — 세션 쿠키가 `api.pullim.ai` host-scoped 면 못 읽어 회원이 튕긴다 → **games 가 로그인 성공 후 자기 host 에 host-only presence 힌트 쿠키 직접 set**(api 는 sibling 쿠키 set 불가). **⚠️ 전제 2 — 정밀 경로 재지정**: games `/api/auth/me` 제거 시 클라 정밀 체크가 **`api.pullim.ai/auth/me`(`credentials:include`)** 를 호출하도록 재배선해야 한다(안 하면 만료/위조 세션을 못 걸러 보호 라우트가 stale 노출). **⚠️ 전제 3 — presence 쿠키 lifecycle 정합**: presence 힌트는 coarse 신호일 뿐 권위 아님 → ❶ 로그인 성공 시 set, ❷ **로그아웃 시 반드시 clear**(games logout 흐름이 presence 쿠키도 삭제 — 안 하면 로그아웃 후에도 coarse 통과), ❸ **세션 만료 시**: presence 가 세션보다 오래 남을 수 있으므로(TTL mismatch) 만료된 stale presence 는 정밀 체크(`/auth/me` 401)가 잡아 redirect+presence clear. presence TTL ≤ 세션 TTL 권장. 이 set/clear/만료 정합을 계약화([P0 설계 TODO] 쿠키 Domain·CSRF 항목과 연동).
...
4. **회원 프로필 필드 — ⚠️ 2026-07-05 정정(grade 분리, spec/05 §5.2⒜⑵·§5.6)**: 원안은 "`grade` + `consent` 를 pullim-api 중앙 수집·검증"이었으나, **`grade` 는 pullim-api 요청에서 제외**한다 — grade 는 콘텐츠 preference(identity 아님)라 **games-side 존치**(중앙 signup grade 미수집, games projection). ✅ **`consent`(만 14세 자가확인)만 중앙 가입 계약**(pullim-web/pullim-api 소유, §5.6). ~~games-local `users.grade` 이관~~ 폐기 — grade 는 games 존치. (games=플랫폼 `games` 패키지, `junior`(초등 주니어) 아님.) 상세: `proc/plan/2026-07-05_HANDOFF-pullim-api-games-me-displayname.md`(displayName 만 pullim-api, grade games-side).
...
6. **sync CSRF 토큰 발급처 대체**(직접 호출 전환의 차단점): 현행 games 는 `GET /api/sync/csrf` 가 double-submit 토큰 쿠키(`pullim-csrf-sync`)를 발급하고 POST `/api/sync` 가 same-origin+토큰 2중 검증한다. 직접 호출 전환에서 games `/api/sync*`(csrf 발급 포함)를 제거하면 **이 토큰을 발급할 엔드포인트가 사라진다** → `api.pullim.ai` 가 **sync(또는 통합 `/games/*`) CSRF 토큰 발급+검증을 제공**해야 한다(cross-subdomain double-submit 성립 모델은 [P0 설계 TODO] ⑨ 와 동일 계약). 발급처 계약 없이 games csrf 라우트만 지우면 sync POST 가 항상 403.
