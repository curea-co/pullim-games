...
본 리포의 Next.js 판단은 `proc/spec/09-기술-환경.md §9.1` 이 최종 권위 — **표준 Next.js 15.5+ (App Router)** 가 설치되어 있고, `node_modules/next/dist/docs/` 는 부재한다 (spec/09 §9.1 표 명시). 본 분기 룰은 `proc/spec/01 §3` "Next.js 룰 — 본 리포 한정 분기" 에 정합화 (2026-05-26 G1 합의, plan-g phase 1.7).
...
- 권위 문서는 `proc/spec/01~10` — 룰 모호하면 spec 우선. 권위 문서 간 충돌은 더 구체적·신규 spec 우선이며, 본 리포 환경 한정 해석은 spec 본문에 인라인으로 정합화하는 것이 1차 (예: spec/01 §3 의 "Next.js 룰 — 본 리포 한정 분기"). spec 으로 표현하기 어려운 운영 룰만 AGENTS.md/CLAUDE.md 에 둔다.
